...
public sealed class ScriptCompilerReportTests
{
    ...
    [Fact]
    public void Compile_EmitsDeterministicReportForEquivalentInputs()
    {
        ...
        using var document = JsonDocument.Parse(SerializeReport(first.Report));
        Assert.Equal(
            new[] { "script", "templates", "sections", "governedResources", "spec", "diagnostics" },
            document.RootElement.EnumerateObject().Select(property => property.Name).ToArray());
    }

    [Fact]
    public void Compile_ReportIncludesSectionBindingsAndGovernedResources()
    {
        var result = Compile(CreateValidJson(new SectionInput("section-a", 1, "First section")));

        Assert.True(result.Success);
        var report = result.Report;
        Assert.Equal("script-demo", report.Script.ScriptId);
        Assert.Single(report.Templates);
        Assert.Single(report.Sections);
        Assert.Equal("title-card-basic", report.Sections[0].TemplateId);
        Assert.Equal("svg-hero-governed", report.Sections[0].AssetIds.Single());
        Assert.Equal("effect-draw-default", report.Sections[0].EffectProfileIds.Single());
        Assert.Equal("reg-main-2026-04", report.GovernedResources.SnapshotId);
        Assert.True(report.Spec.SpecOutputGenerated);
        Assert.NotEmpty(report.Spec.SpecOutputDeterministicKey);
    }

    [Fact]
    public void Compile_EmitsScriptSpecValidationFailedDiagnosticWhenGeneratedSpecIsInvalid()
    {
        ...
        try
        {
            ...
            Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "script.spec.validation.failed");
            Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "semantic.scene_object.asset_ref.type_mismatch");
            Assert.Equal("section-a", result.Diagnostics.First(diagnostic => diagnostic.Code == "semantic.scene_object.asset_ref.type_mismatch").SectionId);
            Assert.False(result.Report.Spec.Success);
            Assert.False(result.Report.Spec.SpecOutputGenerated);
        }
        finally
        {
    ...
    {
        return JsonSerializer.Serialize(report, ReportSerializerOptions);
    }
    ...
    {
        var directoryPath = Path.Combine(Path.GetTempPath(), "whiteboard-script-compiler-report-tests", Guid.NewGuid().ToString("N"));
